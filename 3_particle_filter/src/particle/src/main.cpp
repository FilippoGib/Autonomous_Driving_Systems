...
static constexpr double degrees_to_radiants(double degrees)
{
...
int main(int argc,char **argv)
{
    ...
    pcl::io::loadPCDFile ("/app/Autonomous_Driving_Systems/3_particle_filter/data/map_reflector.pcd", *cloudReflectors); // cloud with just the reflectors
    pcl::io::loadPCDFile ("/app/Autonomous_Driving_Systems/3_particle_filter/data/map_pepperl.pcd", *cloudMap); // total cloud (used for rendering)

    remove("./res.txt");
    // This function locates the reflectors within the map and writes into the file
    createMap(cloudReflectors,"/app/Autonomous_Driving_Systems/3_particle_filter/data/map_data.txt",map_mille);
